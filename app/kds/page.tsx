...
// --- KONFIGURASI BACKEND ---
// Ganti URL ini jika port backend temanmu berbeda (misal: 8000 atau 3001)
const API_URL = "http://localhost:5000/api/kds/orders";

// Komponen Timer untuk memantau durasi pesanan
const OrderTimer = ({ createdAt, status }: { createdAt: any, status: string }) => {
  const [elapsed, setElapsed] = useState(0);

  useEffect(() => {
    if (!createdAt || status === 'ready' || status === 'done') return;
  ...
  if (!createdAt) return <div className="font-mono font-bold text-xl text-gray-300">00:00</div>;
  ...
  return (
    <div className={`font-mono font-bold text-xl ${textColor}`}>
...
export default function KitchenDisplay() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      if (result.success) {
        // Mengasumsikan backend mengirim data pesanan yang belum 'done'
  ...
  // 2. Efek untuk Polling (Update otomatis tiap 5 detik)
  useEffect(() => {
    fetchOrders();
  ...
  // 3. Fungsi Update Status ke Backend
  const handleUpdateStatus = async (id: string | number, currentStatus: string) => {
    ...
    try {
      const response = await fetch(`${API_URL}/${id}`, {
        method: 'PATCH', // Atau 'PUT' sesuai kesepakatan dengan temanmu
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status: nextStatus })
      });

      if (response.ok) {
        fetchOrders(); // Ambil data terbaru setelah update berhasil
      }
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-[#f8f9fa] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 font-bold uppercase tracking-widest text-xs">Menghubungkan ke Dapur...</p>
        </div>
      </div>
    );
  }
      ...
      {orders.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {orders.map((order: any) => (
            <div key={order.id} className="bg-white rounded-[2.5rem] overflow-hidden border border-gray-100 shadow-sm flex flex-col transition-all">
              ...
              <div className="p-8 flex-1">
                <ul className="space-y-4">
                  {order.items.map((item: string, idx: number) => (
                    <li key={idx} className="text-lg font-bold text-gray-700 border-l-4 border-[#F58A27] pl-4 py-1 leading-tight">
                      {item}
                    </li>
                  ))}
                </ul>
              ...
              <div className="p-6 mt-auto">
                <button 
                  ...
                  className={`w-full py-5 rounded-[1.8rem] font-black text-sm uppercase tracking-widest transition-all active:scale-95 ${
                    ...
                    ? 'bg-[#F58A27] text-white hover:bg-[#d4721a]' 
